...
static uint32_t llama_recurrent_safe_ubatch_size(
        const llama_model & model,
        const llama_batch & batch_all,
        uint32_t cur_token,
        uint32_t n_tokens) {
    if (!llm_arch_is_hybrid(model.arch) ||
            n_tokens <= 1 ||
    ...
    const llama_seq_id cur_seq_id = batch_all.seq_id[cur_token][0];

    uint32_t n_tokens_same_seq = 1;
    while (n_tokens_same_seq < n_tokens) {
        const uint32_t idx = cur_token + n_tokens_same_seq;

        if (batch_all.n_seq_id[idx] <= 0 || batch_all.seq_id[idx] == nullptr) {
            break;
        }

        if (batch_all.seq_id[idx][0] != cur_seq_id) {
            break;
        }
    ...
    std::unordered_set<llama_seq_id> prefix_seen_seq_ids;
    prefix_seen_seq_ids.reserve(n_tokens);

    uint32_t n_tokens_unique_prefix = 0;
    while (n_tokens_unique_prefix < n_tokens) {
        const uint32_t idx = cur_token + n_tokens_unique_prefix;
        ...
        if (!prefix_seen_seq_ids.insert(seq_id_i).second) {
            break;
        }

        ++n_tokens_unique_prefix;
    }

    const uint32_t selected = std::max(n_tokens_same_seq, n_tokens_unique_prefix);

    if (selected > 1 && n_tokens_unique_prefix > n_tokens_same_seq) {
        char seq_pattern[256];
        int offset = 0;
        const uint32_t limit = std::min<uint32_t>(n_tokens, 16);

        for (uint32_t i = 0; i < limit && offset < (int) sizeof(seq_pattern); ++i) {
            const uint32_t idx = cur_token + i;
            const llama_seq_id seq_id_i =
                    batch_all.n_seq_id[idx] > 0 && batch_all.seq_id[idx] != nullptr
                    ? batch_all.seq_id[idx][0]
                    : -9999;

            const int written = snprintf(seq_pattern + offset,
                    sizeof(seq_pattern) - offset,
                    "%s%d",
                    i == 0 ? "" : ",",
                    (int) seq_id_i);
        ...
        if (offset >= (int) sizeof(seq_pattern)) {
            seq_pattern[sizeof(seq_pattern) - 1] = '\0';
        } else {
            seq_pattern[offset] = '\0';
        }

        LLAMA_LOG_WARN("QWEN36_MIXED_UBATCH_DIAG_V1: cur_token=%u requested=%u selected=%u same_seq=%u unique_prefix=%u any_diff=%d has_dup=%d pattern_first16=[%s] policy=max_same_or_unique_prefix\n",
                cur_token,
                n_tokens,
                selected,
                n_tokens_same_seq,
...
static int llama_decode_internal(
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; ) {
        ...
        const uint32_t n_tokens_safe = llama_recurrent_safe_ubatch_size(model, batch_all, cur_token, n_tokens);
        if (n_tokens_safe < n_tokens) {
            n_tokens = n_tokens_safe;

            if (!warned_qnext_mixed_repeat) {
                LLAMA_LOG_INFO("%s: qwen3next recurrent-safe ubatch split active for mixed sequence batch\n", __func__);
