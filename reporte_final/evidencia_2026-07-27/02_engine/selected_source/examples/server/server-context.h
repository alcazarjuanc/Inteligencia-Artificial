...
struct server_slot {
    ...
    int32_t forced_checkpoint_min_step = -1;
    int32_t n_before_user = -1;
    std::string n_before_user_tag;
    std::vector<int32_t> n_before_user_boundaries;
    std::vector<std::string> n_before_user_boundary_tags;
    bool n_before_user_known = false;

    bool pending_checkpoint_metadata_valid = false;
    bool pending_checkpoint_safe_boundary_created = false;
    std::string pending_checkpoint_type;
    std::string pending_checkpoint_source;
    bool pending_periodic_safe_boundary_checkpoint = false;
    int32_t pending_periodic_safe_boundary_min_pos = -1;
    ...
    // Scope key used to prevent cross-agent/repo cache reuse.
    // cache_scope_key describes the prompt currently represented by slot.cache_tokens.
    // task_scope_key describes the prompt currently being loaded into this slot.
...
struct server_context {
    ...
    bool slot_affinity_by_repo_scope = false;
