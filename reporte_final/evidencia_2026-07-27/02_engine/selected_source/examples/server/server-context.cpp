...
// Slot-scoped recurrent/KV cleanup for server-side cache recovery.
// This intentionally avoids global kv_cache_clear(), so it is safe for np > 1.
static bool clear_slot_recurrent_kv_state(server_slot & slot, llama_context * ctx) {
    bool target_cleared = llama_kv_cache_seq_rm(ctx, slot.id, -1, -1);
    bool companion_cleared = true;
    ...
    if (slot.spec != nullptr) {
        ...
        if (auto * ctx_companion = common_speculative_get_companion_ctx(slot.spec); ctx_companion != nullptr) {
            companion_cleared = llama_kv_cache_seq_rm(ctx_companion, slot.id, -1, -1);
        }
    }

    return target_cleared && companion_cleared;
}

// Try a partial trim first. If the recurrent backend refuses partial removal,
// clear only this slot and force an isolated re-prefill path.
static bool trim_slot_kv_or_clear_for_refill(server_slot & slot, llama_context * ctx, llama_pos p0) {
    ...
    clear_slot_recurrent_kv_state(slot, ctx);
    return false;
}
...
bool server_context::load_model(const gpt_params& params_) {
    ...
    // QWEN36_EXPERIMENT_MTP_PARALLEL_SLOTS_NO_GUARD_V1:
    // Allow MTP with parallel server slots. This is experimental and relies on
    // per-slot speculative contexts/checkpoints instead of disabling MTP globally.
    if (false && params_base.has_mtp && params_base.n_parallel > 1) {
...
void server_context::init() {
    ...
    for (int i = 0; i < params_base.n_parallel; i++) {
        ...
        if (can_spec && requested_spec) {
            auto spec_params_slot = params_base.speculative;

            if (server_speculative_has_mtp(spec_params_slot) && params_base.n_parallel > 1) {
                spec_params_slot.n_ctx = n_ctx_slot;
                SRV_INF(
                    "QWEN36_EXPERIMENT_MTP_PARALLEL_SLOT_CTX_V1: slot=%d n_parallel=%d base_n_ctx=%d slot_n_ctx=%d\n",
                    slot.id,
            ...
            slot.spec = common_speculative_init(spec_params_slot, slot.ctx);
            if (slot.spec) {
...
void server_slot::prompt_load(server_prompt_cache& prompt_cache, const server_tokens& tokens) {
    if (!server_cached_prompt.checkpoints.empty()) {
        LLAMA_LOG_INFO("PROMPT_CACHE_LOAD_SKIP_RECURRENT_SLOT_CHECKPOINTS: slot=%d, tokens=%d, checkpoints=%zu\n",
                id,
                (int)server_cached_prompt.tokens.size(),
                server_cached_prompt.checkpoints.size());
        return;
    }
...
void server_slot::reset() {
    ...
    checkpoint_pos = 0;
    forced_checkpoint_min_step = -1;
    n_before_user = -1;
    n_before_user_boundaries.clear();
    n_before_user_known = false;
...
static std::string server_normalize_repo_scope_key(std::string scope) {
    while (!scope.empty() && std::isspace(static_cast<unsigned char>(scope.back()))) {
        scope.pop_back();
    }

    while (!scope.empty()) {
        const char c = scope.back();
        if (c == '.' || c == ',' || c == ';' || c == ':' || c == ')' || c == ']' || c == '}') {
            scope.pop_back();
            continue;
        }
...
static std::string server_extract_repo_scope_from_text(const std::string & text) {
    const std::string marker = "/home/alcazarjuanc/ai-stack/agents/openclaude/test-repos/";
    size_t pos = text.find(marker);

    if (pos != std::string::npos) {
        pos += marker.size();
        size_t end = text.find_first_of(" \n\r\t\"'`<>", pos);
        std::string scope = text.substr(pos, end == std::string::npos ? std::string::npos : end - pos);
        scope = server_normalize_repo_scope_key(scope);
        if (!scope.empty()) {
            return scope;
        }
    }

    pos = text.find("oc-agent-");
    if (pos != std::string::npos) {
        size_t end = text.find_first_of(" \n\r\t\"'`<>/", pos);
        std::string scope = text.substr(pos, end == std::string::npos ? std::string::npos : end - pos);
        scope = server_normalize_repo_scope_key(scope);
...
static std::string server_scope_affinity_key_v1(const std::string & scope) {
    const std::string marker = "agent-";
    const size_t pos = scope.find(marker);

    if (pos == std::string::npos) {
        return "";
    }

    const size_t key_pos = pos + marker.size();
    if (key_pos >= scope.size()) {
        return "";
    }

    const char key = scope[key_pos];
...
static std::string server_extract_repo_scope_from_task_data(const json & data) {
    const auto it_prompt = data.find("prompt");
    if (it_prompt != data.end()) {
        if (it_prompt->is_string()) {
            return server_extract_repo_scope_from_text(it_prompt->get<std::string>());
        }

        if (it_prompt->is_array()) {
            for (const auto & item : *it_prompt) {
                if (item.is_string()) {
                    const auto scope = server_extract_repo_scope_from_text(item.get<std::string>());
                    if (!scope.empty()) {
                        return scope;
                    }
                }
    ...
    return server_extract_repo_scope_from_text(dumped);
}
...
server_slot* server_context::get_available_slot(const server_task& task) {
    ...
    const std::string task_scope_key = server_extract_repo_scope_from_task_data(task.data);
    const std::string task_key = server_scope_affinity_key_v1(task_scope_key);

    const bool SLOT_AFFINITY_BY_REPO_SCOPE_STRICT_V1 = slot_affinity_by_repo_scope;

    if (SLOT_AFFINITY_BY_REPO_SCOPE_STRICT_V1 && !task_key.empty()) {
        auto slot_effective_scope_key = [](const server_slot & slot) -> std::string {
            if (!slot.cache_scope_key.empty()) {
                return slot.cache_scope_key;
            }
            if (!slot.task_scope_key.empty()) {
                return slot.task_scope_key;
            }
            return {};
        };

        for (server_slot& slot : slots) {
            const std::string effective_scope_key = slot_effective_scope_key(slot);
            const std::string effective_key = server_scope_affinity_key_v1(effective_scope_key);

            if (effective_key == task_key) {
                if (!slot.available()) {
                    LLAMA_LOG_WARN("SLOT_AFFINITY_BY_REPO_SCOPE_STRICT_V2: task=%d key=%s slot=%d action=defer reason=busy effective_scope=%s cache_scope=%s task_scope=%s\n",
                            task.id,
                            task_key.c_str(),
                            slot.id,
                            effective_scope_key.c_str(),
                            slot.cache_scope_key.c_str(),
                            slot.task_scope_key.c_str());
                    return nullptr;
                }

                LLAMA_LOG_WARN("SLOT_AFFINITY_BY_REPO_SCOPE_STRICT_V2: task=%d key=%s slot=%d action=select reason=match effective_scope=%s cache_scope=%s task_scope=%s\n",
                        task.id,
                        task_key.c_str(),
                        slot.id,
                        effective_scope_key.c_str(),
        ...
        for (server_slot& slot : slots) {
            ...
            if (effective_key.empty() &&
                    slot.cache_scope_key.empty() &&
                    slot.task_scope_key.empty() &&
                    slot.cache_tokens.empty()) {
                LLAMA_LOG_WARN("SLOT_AFFINITY_BY_REPO_SCOPE_STRICT_V2: task=%d key=%s slot=%d action=select reason=init\n",
                        task.id,
                        task_key.c_str(),
                        slot.id);
                return &slot;
        ...
        LLAMA_LOG_WARN("SLOT_AFFINITY_BY_REPO_SCOPE_STRICT_V2: task=%d key=%s action=defer reason=no_slot\n",
                task.id,
                task_key.c_str());
        return nullptr;
    }
    ...
    const bool SLOT_AFFINITY_BY_REPO_SCOPE_V1 = slot_affinity_by_repo_scope;

    if (SLOT_AFFINITY_BY_REPO_SCOPE_V1 && ret == nullptr && !task_scope_key.empty()) {
        for (server_slot& slot : slots) {
            if (!slot.available()) {
                continue;
            }

            if (!slot.cache_scope_key.empty() && slot.cache_scope_key == task_scope_key) {
                ret = &slot;
                LLAMA_LOG_WARN("SLOT_AFFINITY_BY_REPO_SCOPE_V1: selected existing scoped slot=%d task=%d scope=%s reason=scope_match\n",
                        slot.id,
                        task.id,
                        task_scope_key.c_str());
                break;
            }
        }
        ...
        if (ret == nullptr) {
            for (server_slot& slot : slots) {
                ...
                if (slot.cache_scope_key.empty() && slot.cache_tokens.empty()) {
                    ret = &slot;
                    LLAMA_LOG_WARN("SLOT_AFFINITY_BY_REPO_SCOPE_V1: selected empty slot=%d task=%d scope=%s reason=scope_init\n",
                            slot.id,
                            task.id,
                            task_scope_key.c_str());
                    break;
    ...
    if (ret == nullptr && slot_prompt_similarity != 0.0f) {
        ...
        for (server_slot& slot : slots) {
            ...
            if (!task_scope_key.empty()) {
                if (slot.cache_scope_key.empty() || slot.cache_scope_key != task_scope_key) {
                    LOG_VERBOSE("skip slot similarity due to cache scope mismatch", {
                        {"id_slot", slot.id},
                        {"slot_scope", slot.cache_scope_key},
                        {"task_scope", task_scope_key},
                    });
                    continue;
                }
            }
    ...
    if (ret) {
        ...
        if (prompt_cache && !prompt_cache->states.empty()) {
            const bool has_recurrent = llama_model_has_recurrent(model);
            const bool prompt_cache_scope_mismatch =
                !task_scope_key.empty() &&
                !ret->cache_scope_key.empty() &&
                ret->cache_scope_key != task_scope_key;

            if (prompt_cache_scope_mismatch) {
                LLAMA_LOG_WARN("PROMPT_CACHE_LOAD_SKIP_SCOPE_MISMATCH: recurrent=%d, selected_slot=%d, task_id=%d, old_scope=%s, new_scope=%s, prompt_cache_states=%zu\n",
                        has_recurrent ? 1 : 0,
                        ret->id,
                        task.id,
                        ret->cache_scope_key.c_str(),
                        task_scope_key.c_str(),
                        prompt_cache->states.size());
            } else {
                LLAMA_LOG_WARN("PROMPT_CACHE_LOAD_BEGIN: recurrent=%d, selected_slot=%d, task_id=%d, slot_cache_tokens=%d, task_tokens=%d, prompt_cache_states=%zu, f_keep=%.3f, cache_ram_similarity=%.3f\n",
                        has_recurrent ? 1 : 0,
                        ret->id,
                        task.id,
                        (int) tokens.size(),
                        (int) task.tokens.size(),
                        prompt_cache->states.size(),
                        f_keep,
            ...
            LLAMA_LOG_WARN("PROMPT_CACHE_LOAD_AFTER: selected_slot=%d, task_id=%d, kept=%d, discarded=%d, loaded_tokens=%d, loaded_checkpoints=%zu\n",
                    ret->id,
                    task.id,
                    ret->server_cached_prompt.n_kept_prompt,
                    ret->server_cached_prompt.n_discarded_prompt,
                    (int) ret->server_cached_prompt.tokens.size(),
                    ret->server_cached_prompt.checkpoints.size());

            if (has_recurrent && ret->server_cached_prompt.tokens.empty() && !ret->server_cached_prompt.checkpoints.empty()) {
                LLAMA_LOG_WARN("PROMPT_CACHE_LOAD_DROP_EMPTY_RECURRENT: selected_slot=%d, task_id=%d, dropping %zu checkpoints because loaded_tokens=0\n",
                        ret->id,
                        task.id,
                        ret->server_cached_prompt.checkpoints.size());
...
static std::vector<std::pair<int32_t, std::string>> prompt_get_n_before_users(
        const json & message_spans,
        const std::string & prompt,
        const llama_vocab * vocab) {
    std::vector<std::pair<int32_t, std::string>> result;

    constexpr bool MESSAGE_SPANS_USER_END_BOUNDARY_V1 = true;

    auto add_user_boundary = [&](const char * tag, int32_t byte_pos) {
        if (byte_pos < 0 || (size_t) byte_pos > prompt.size()) {
            return;
        }

        const std::string prefix = prompt.substr(0, (size_t) byte_pos);
        const auto tokens = common_tokenize(vocab, prefix, false, true);
        const int32_t n_boundary = (int32_t) tokens.size();

        if (n_boundary > 0 && (result.empty() || result.back().first != n_boundary)) {
            result.emplace_back(n_boundary, std::string(tag));
            SRV_INF("message_spans: user message boundary: tag=%s byte_pos=%d n_boundary=%d\n",
                    tag, byte_pos, n_boundary);
        }
    };
    ...
    for (const auto & span : message_spans) {
        ...
        add_user_boundary("user_start", byte_pos);

        if (MESSAGE_SPANS_USER_END_BOUNDARY_V1) {
            const int32_t explicit_end = json_value(span, "end", json_value(span, "end_pos", -1));
            const int32_t explicit_len = json_value(span, "len", json_value(span, "length", -1));

            int32_t end_byte_pos = -1;
            if (explicit_end >= 0) {
                end_byte_pos = explicit_end;
            } else if (byte_pos >= 0 && explicit_len >= 0) {
                end_byte_pos = byte_pos + explicit_len;
            }

            add_user_boundary("user_end", end_byte_pos);
        }
    }
...
bool server_context::launch_slot_with_task(server_slot& slot, server_task& task) {
    ...
    slot.task_scope_key = server_extract_repo_scope_from_task_data(data);

    const llama_vocab* vocab = llama_model_get_vocab(model);

    slot.n_before_user = -1;
    slot.n_before_user_tag.clear();
    slot.n_before_user_boundaries.clear();
    slot.n_before_user_boundary_tags.clear();
    slot.n_before_user_known = false;
    slot.pending_checkpoint_metadata_valid = false;
    slot.pending_checkpoint_safe_boundary_created = false;
    slot.pending_checkpoint_type.clear();
    slot.pending_checkpoint_source.clear();
    slot.pending_periodic_safe_boundary_checkpoint = false;
    slot.pending_periodic_safe_boundary_min_pos = -1;

    {
        const auto message_spans = json_value(data, "message_spans", json::array());
        const auto prompt = json_value(data, "prompt", json());

        if (prompt.is_string() && message_spans.is_array()) {
            const auto user_boundaries = prompt_get_n_before_users(
                message_spans,
            ...
            for (const auto & boundary : user_boundaries) {
                slot.n_before_user_boundaries.push_back(boundary.first);
                slot.n_before_user_boundary_tags.push_back(boundary.second);
            }
        }

        if (!slot.n_before_user_boundaries.empty()) {
            slot.n_before_user = slot.n_before_user_boundaries.front();
            slot.n_before_user_tag = !slot.n_before_user_boundary_tags.empty()
                ? slot.n_before_user_boundary_tags.front()
                : "user_boundary";
            slot.n_before_user_known = true;
            SRV_INF("message_spans: slot=%d task=%d n_before_user=%d tag=%s boundaries=%zu\n",
                    slot.id, task.id, slot.n_before_user, slot.n_before_user_tag.c_str(), slot.n_before_user_boundaries.size());
        }
    }
...
bool server_context::system_prompt_set(const std::string& sys_prompt) {
    ...
    for (server_slot& slot : slots) {
        ...
        slot.cache_scope_key.clear();
        slot.task_scope_key.clear();
...
void  server_context::create_checkpoint_at_interval(server_slot & slot, const gpt_params & params_base) {
    if (params_base.do_checkpoint && params_base.ctx_checkpoints_interval > 0) {
        ...
        int32_t checkpoint_interval_base_pos = slot.checkpoint_pos;
        if (params_base.ctx_checkpoints_safe_boundary_only && !slot.server_cached_prompt.checkpoints.empty()) {
            checkpoint_interval_base_pos = (int32_t) slot.server_cached_prompt.checkpoints.back().n_tokens;
        }

        if (checkpoint_interval_base_pos + params_base.ctx_checkpoints_interval <= 1 + pos) {
            if (params_base.ctx_checkpoints_safe_boundary_only) {
                if (!slot.pending_periodic_safe_boundary_checkpoint) {
                    int32_t previous_checkpoint_pos = checkpoint_interval_base_pos;

                    int32_t safe_boundary_back_tolerance = params_base.ctx_checkpoints_interval > 0
                        ? params_base.ctx_checkpoints_interval / 16
                        : 0;
                    if (safe_boundary_back_tolerance > 256) {
                        safe_boundary_back_tolerance = 256;
                    }
                    if (safe_boundary_back_tolerance < 0) {
                        safe_boundary_back_tolerance = 0;
                    }

                    int32_t safe_boundary_min_pos = 1 + (int32_t) pos - safe_boundary_back_tolerance;
                    if (safe_boundary_min_pos < 0) {
                        safe_boundary_min_pos = 0;
                    }

                    int32_t min_step = params_base.ctx_checkpoints_interval > 0
                        ? params_base.ctx_checkpoints_interval / 32
                        : 128;
                    if (min_step < 128) {
                        min_step = 128;
                    }
                    ...
                    int32_t selected_boundary = -1;
                    std::string selected_boundary_tag = "user_boundary";

                    for (size_t bi = 0; bi < slot.n_before_user_boundaries.size(); ++bi) {
                        const int32_t boundary = slot.n_before_user_boundaries[bi];

                        if (boundary < safe_boundary_min_pos) {
                            continue;
                        }
                        if (boundary > 1 + (int32_t) pos) {
                            continue;
                        }

                        selected_boundary = boundary;
                        selected_boundary_tag = bi < slot.n_before_user_boundary_tags.size()
                            ? slot.n_before_user_boundary_tags[bi]
                            : "user_boundary";
                    }

                    if (selected_boundary > 0) {
                        const int32_t gap = selected_boundary - previous_checkpoint_pos;

                        SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_BOUNDARY_DECISION_V1: slot=%d task=%d pos=%d selected_boundary=%d tag=%s previous_checkpoint=%d gap=%d min_step=%d min_boundary=%d back_tolerance=%d\n",
                                slot.id,
                                slot.id_task,
                                (int) pos,
                                selected_boundary,
                                selected_boundary_tag.c_str(),
                                previous_checkpoint_pos,
                                gap,
                                min_step,
                                safe_boundary_min_pos,
                                safe_boundary_back_tolerance);

                        if (gap >= min_step) {
                            slot.n_before_user = selected_boundary;
                            slot.n_before_user_tag = selected_boundary_tag;
                            slot.n_before_user_known = true;

                            slot.pending_checkpoint_metadata_valid = true;
                            slot.pending_checkpoint_safe_boundary_created = true;
                            slot.pending_checkpoint_type = selected_boundary_tag;
                            slot.pending_checkpoint_source = "message_spans_periodic";
                            ...
                            if (created) {
                                ...
                                SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_BOUNDARY_DECISION_V1: slot=%d task=%d action=create selected_boundary=%d tag=%s source=message_spans_periodic\n",
                                        slot.id,
                                        slot.id_task,
                                        selected_boundary,
                                        selected_boundary_tag.c_str());
                                ...
                                SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_BOUNDARY_DECISION_V1: slot=%d task=%d action=defer reason=create_checkpoint_returned_false selected_boundary=%d tag=%s next_retry_after_interval=%d\n",
                                        slot.id,
                                        slot.id_task,
                                        selected_boundary,
                                        selected_boundary_tag.c_str(),
                                        params_base.ctx_checkpoints_interval);
                            }
                        } else {
                            SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_BOUNDARY_DECISION_V1: slot=%d task=%d action=skip reason=gap_too_small selected_boundary=%d gap=%d min_step=%d\n",
                                    slot.id,
                                    slot.id_task,
                                    selected_boundary,
                                    gap,
                    ...
                    if (selected_boundary <= 0) {
                        slot.pending_periodic_safe_boundary_checkpoint = true;
                        slot.pending_periodic_safe_boundary_min_pos = safe_boundary_min_pos;
                        slot.checkpoint_pos = pos;

                        SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_PENDING_V1: slot=%d task=%d pos=%d min_boundary=%d back_tolerance=%d interval=%d action=pending_next_user_boundary\n",
                                slot.id,
                                slot.id_task,
                                (int) pos,
                                slot.pending_periodic_safe_boundary_min_pos,
                                safe_boundary_back_tolerance,
                                params_base.ctx_checkpoints_interval);
                    }
                }
...
void server_context::apply_checkpoint(server_slot & slot) {
    ...
    // For recurrent/hybrid models, also search checkpoints when KV was cleared
    // but cache_tokens still reflects a previous decode and the new prompt covers
    // that cached prefix. This matches the qwen3next checkpoint-reuse workaround.
    const bool full_match_kv_lost =
        has_recurrent &&
        kv_empty &&
        slot.n_past > 0 &&
        slot.n_past == (int) slot.cache_tokens.n_tokens() &&
    ...
    if ((slot.n_past > 0 && slot.n_past < slot.cache_tokens.n_tokens()) || full_match_kv_lost) {
        if (has_recurrent || pos_min >= pos_min_thold) {
            ...
            if (do_reset) {
                if (has_recurrent) {
                    SLT_WRN(slot, "no usable hybrid/recurrent checkpoint; clearing slot recurrent/KV state + re-prefilling (n_past was %d, n_past_prompt was %d, seq_id = %d)\n",
                        (int)slot.n_past, (int)slot.n_past_prompt, slot.id);

                    const bool slot_cleared = clear_slot_recurrent_kv_state(slot, ctx);

                    if (!slot_cleared) {
                        SLT_WRN(slot, "%s", "slot recurrent/KV clear reported incomplete clear; continuing with isolated re-prefill\n");
                    }
    ...
    {
        // LLAMA_UPSTREAM: erase any checkpoints with pos_max > pos_next
        for (auto it = slot.server_cached_prompt.checkpoints.begin(); it != slot.server_cached_prompt.checkpoints.end();) {
            const auto & cur = *it;
            if (cur.pos_max > pos_next) {
                SLT_WRN(slot, "erased invalidated context checkpoint (pos_min = %d, pos_max = %d, pos_next = %d, size = %.3f MiB)\n", cur.pos_min, cur.pos_max, pos_next, (float)cur.data.size() / 1024 / 1024);
...
bool server_context::create_checkpoint(server_slot & slot) {
    ...
    const bool has_recurrent = llama_model_has_recurrent(llama_get_model(slot.ctx));
    const int32_t checkpoint_min_tokens = has_recurrent ? 4 : 64;
    do_checkpoint = do_checkpoint && (pos_min >= 0 && slot.cache_tokens.n_tokens() >= checkpoint_min_tokens);

    // LLAMA_UPSTREAM + MARKAALONZO_LOCAL:
    // Normal interval checkpoints should respect ctx_checkpoints_interval.
    // Forced prompt-tail checkpoints (slot.do_checkpoint) must bypass spacing so the
    // server can restore close to the end of tool-call prompts instead of falling back
    // to an older checkpoint and re-prefilling thousands of tokens.
    const bool forced_prompt_tail_checkpoint = slot.do_checkpoint;

    // Consume the one-shot forced checkpoint flag here. Without this reset, a single
    // prompt-tail checkpoint request can turn into checkpoint-per-token behavior.
    slot.do_checkpoint = false;

    // Forced prompt-tail checkpoints bypass the normal large interval, but they should
    // not create near-duplicate checkpoints only a few tokens apart.
    const int32_t forced_checkpoint_min_step =
        slot.forced_checkpoint_min_step >= 0 ? slot.forced_checkpoint_min_step : 256;

    slot.forced_checkpoint_min_step = -1;

    const bool forced_prompt_tail_too_close =
        forced_prompt_tail_checkpoint &&
        !slot.server_cached_prompt.checkpoints.empty() &&
        slot.cache_tokens.n_tokens() <= slot.server_cached_prompt.checkpoints.back().n_tokens + forced_checkpoint_min_step;

    do_checkpoint = do_checkpoint && !forced_prompt_tail_too_close && (
        forced_prompt_tail_checkpoint ||
        slot.server_cached_prompt.checkpoints.empty() ||
        params_base.ctx_checkpoints_interval <= 0 ||
        slot.cache_tokens.n_tokens() > slot.server_cached_prompt.checkpoints.back().n_tokens + params_base.ctx_checkpoints_interval
    );

    if (forced_prompt_tail_checkpoint) {
        const int64_t candidate_tokens = slot.cache_tokens.n_tokens();
        const bool has_previous_checkpoint = !slot.server_cached_prompt.checkpoints.empty();
        const int64_t last_checkpoint_tokens = has_previous_checkpoint
            ? slot.server_cached_prompt.checkpoints.back().n_tokens
            : -1;
        const int64_t checkpoint_gap = has_previous_checkpoint
            ? candidate_tokens - last_checkpoint_tokens
            : candidate_tokens;
        const char * checkpoint_action = do_checkpoint ? "create" : "skip";
        const char * checkpoint_reason = forced_prompt_tail_too_close
            ? "gap_too_small"
            : (do_checkpoint ? "gap_ok" : "blocked_by_checkpoint_policy");

        SRV_INF("message_spans: boundary checkpoint decision slot=%d task=%d candidate=%" PRId64 " last_checkpoint=%" PRId64 " gap=%" PRId64 " min_step=%d action=%s reason=%s checkpoint_count=%zu checkpoint_limit=%d\n",
                slot.id,
                slot.id_task,
                candidate_tokens,
                last_checkpoint_tokens,
                checkpoint_gap,
                forced_checkpoint_min_step,
                checkpoint_action,
                checkpoint_reason,
    ...
    if (do_checkpoint) {
        ...
        if (slot.pending_checkpoint_metadata_valid) {
            cur.safe_boundary_created = slot.pending_checkpoint_safe_boundary_created;
            cur.checkpoint_type = slot.pending_checkpoint_type.empty()
                ? "user_boundary"
                : slot.pending_checkpoint_type;
            cur.checkpoint_source = slot.pending_checkpoint_source.empty()
                ? "message_spans"
                : slot.pending_checkpoint_source;
        } else {
            cur.safe_boundary_created = false;
            cur.checkpoint_type = "recurrent_base";
            cur.checkpoint_source = "base_checkpoint";
        }

        SRV_INF("checkpoint metadata: slot=%d task=%d n_tokens=%" PRId64 " safe_boundary=%d type=%s source=%s\n",
                slot.id,
                slot.id_task,
                cur.n_tokens,
                cur.safe_boundary_created ? 1 : 0,
                cur.checkpoint_type.c_str(),
                cur.checkpoint_source.c_str());
            ...
            if (slot.state == SLOT_STATE_IDLE && slot.command == SLOT_COMMAND_LOAD_PROMPT) {
                ...
                if (prompt_tokens.empty() || slot.n_prompt_tokens == 0) {
                    ...
                    if (slot.embedding) {
                        ...
                        if (!slot.task_scope_key.empty()) {
                            const bool has_slot_cache =
                                !slot.cache_tokens.empty() ||
                                !slot.server_cached_prompt.checkpoints.empty() ||
                                !slot.server_cached_prompt.data.empty();

                            if (has_slot_cache && slot.cache_scope_key != slot.task_scope_key) {
                                LLAMA_LOG_WARN("CACHE_SCOPE_MISMATCH: slot=%d task=%d old_scope=%s new_scope=%s; clearing slot-local cache before prompt reuse\n",
                                    slot.id,
                                    slot.id_task,
                                    slot.cache_scope_key.c_str(),
                                    slot.task_scope_key.c_str());
                            ...
                            slot.cache_scope_key = slot.task_scope_key;
                        }
                        ...
                        if (!slot.params.cache_prompt) {
                            ...
                            {
                                LLAMA_LOG_WARN("COMMON_PREFIX_MISMATCH: recurrent=%d, slot=%d, task=%d, n_past=%d, n_past_prompt=%d, cache_size=%d, prompt_tokens=%d, exact=%d, string_prefix_cache=%d, string_prefix_prompt=%d, nonexact_cache=%d, nonexact_prompt=%d\n",
                                        llama_model_has_recurrent(model) ? 1 : 0,
                                        slot.id,
                                        slot.id_task,
                                        (int) slot.n_past,
                                        (int) slot.n_past_prompt,
                                        (int) slot.cache_tokens.size(),
                                        (int) prompt_tokens.size(),
                                        (int) n_past0,
                                        (int) prefix.first,
                                        (int) prefix.second,
                                        (int) prefix_nonexact.first,
                                        (int) prefix_nonexact.second);
                                ...
                                if (llama_model_has_recurrent(model)) {
                                    bool partial_reprefill_used = false;
                                    const int64_t exact_prefix = (int64_t) n_past0;
                                    const bool single_slot_mode = params_base.n_parallel <= 1;

                                    const int64_t min_reprefill_window =
                                            params_base.ctx_checkpoints_interval > 0
                                            ? (int64_t) params_base.ctx_checkpoints_interval
                                            : 0;
                                    const int64_t max_partial_checkpoint =
                                            params_base.ctx_checkpoints_safe_boundary_only
                                            ? exact_prefix - min_reprefill_window
                                            : exact_prefix;

                                    auto is_partial_candidate = [&](const server_prompt_checkpoint & ckpt) {
                                        if (!ckpt.safe_boundary_created) {
                                            return false;
                                        }

                                        if (params_base.ctx_checkpoints_safe_boundary_only) {
                                            if (ckpt.checkpoint_source != "message_spans_periodic") {
                                                return false;
                                            }

                                            if (ckpt.n_tokens <= 0 || ckpt.n_tokens > max_partial_checkpoint) {
                                                return false;
                                            }

                                            return ckpt.checkpoint_type == "user_start" ||
                                                   ckpt.checkpoint_type == "user_end" ||
                                                   ckpt.checkpoint_type == "user_boundary";
                                        }

                                        const bool message_span_checkpoint =
                                                ckpt.checkpoint_source == "message_spans" ||
                                                ckpt.checkpoint_source == "message_spans_periodic";
                                        if (!message_span_checkpoint) {
                                            return false;
                                        }
                                        ...
                                        return ckpt.checkpoint_type == "user_start" &&
                                                slot.n_before_user > 0 &&
                                                ckpt.n_tokens == slot.n_before_user;
                                    };

                                    // QWEN36_SKIP_PARTIAL_ON_TOOL_EDIT_MISMATCH_V1
                                    auto mismatch_window_contains_tool_edit = [&](const server_tokens & tokens, int32_t idx) {
                                        const int32_t start = std::max<int32_t>(0, idx - 80);
                                        const int32_t end = std::min<int32_t>((int32_t) tokens.size(), idx + 160);
                                        std::string text;

                                        for (int32_t i = start; i < end; ++i) {
                                            text += common_token_to_piece(ctx, tokens[i], true);
                                        }
                                        ...
                                        return text.find("<tool_call>") != std::string::npos ||
                                                ...
                                                text.find("new_string") != std::string::npos ||
                                                text.find("replace_all") != std::string::npos ||
                                                text.find("file_path") != std::string::npos ||
                                                text.find("Update(") != std::string::npos ||
                                                text.find("Edit(") != std::string::npos ||
                                                text.find("Write(") != std::string::npos;
                                    };

                                    const bool tool_edit_mismatch =
                                            mismatch_window_contains_tool_edit(slot.cache_tokens, slot.n_past) ||
                                            mismatch_window_contains_tool_edit(prompt_tokens, slot.n_past_prompt);

                                    if (tool_edit_mismatch) {
                                        LLAMA_LOG_WARN("COMMON_PREFIX_MISMATCH_TOOL_EDIT_WINDOW_POLICY_V1: slot=%d task=%d exact_prefix=%" PRId64 " n_past=%d n_past_prompt=%d cache_size=%d prompt_tokens=%d action=evaluate_safe_boundary_window min_window=%d reason=tool_edit_mismatch\n",
                                                slot.id,
                                                slot.id_task,
                                                exact_prefix,
                                                (int) slot.n_past,
                                    ...
                                    auto it_partial = slot.server_cached_prompt.checkpoints.rend();

                                    if (!tool_edit_mismatch || params_base.ctx_checkpoints_safe_boundary_only) {
                                        it_partial = std::find_if(
                                                slot.server_cached_prompt.checkpoints.rbegin(),
                                                slot.server_cached_prompt.checkpoints.rend(),
                                                is_partial_candidate);
                                    }

                                    if (it_partial != slot.server_cached_prompt.checkpoints.rend()) {
                                        const int64_t selected_checkpoint = it_partial->n_tokens;
                                        const int tokens_to_prefill = (int) prompt_tokens.size() - (int) selected_checkpoint;
                                        const int64_t t_start_partial = ggml_time_us();
                                        const size_t checkpoint_size = it_partial->data.size();

                                        LLAMA_LOG_WARN("COMMON_PREFIX_MISMATCH_RECURRENT_PARTIAL_REPREFILL_MIN_V1_CANDIDATE: slot=%d task=%d mode=%s exact_prefix=%" PRId64 " selected_checkpoint=%" PRId64 " prompt_tokens=%d tokens_to_prefill=%d type=%s source=%s n_before_user=%d\n",
                                                slot.id,
                                                slot.id_task,
                                                params_base.ctx_checkpoints_safe_boundary_only ? "safe_boundary_window" : (single_slot_mode ? "nearest" : "n_before_user"),
                                                exact_prefix,
                                                selected_checkpoint,
                                                (int) prompt_tokens.size(),
                                                tokens_to_prefill,
                                                it_partial->checkpoint_type.c_str(),
                                                it_partial->checkpoint_source.c_str(),
                                                slot.n_before_user);

                                        if (params_base.ctx_checkpoints_safe_boundary_only) {
                                            LLAMA_LOG_WARN("QWEN36_SAFE_BOUNDARY_ONLY_PARTIAL_WINDOW_V1: slot=%d task=%d exact_prefix=%" PRId64 " min_window=%" PRId64 " max_checkpoint=%" PRId64 " selected_checkpoint=%" PRId64 " actual_window=%" PRId64 "\n",
                                                    slot.id,
                                                    slot.id_task,
                                                    exact_prefix,
                                                    min_reprefill_window,
                                                    max_partial_checkpoint,
                                                    selected_checkpoint,
                                                    exact_prefix - selected_checkpoint);
                                        }

                                        const size_t n = llama_state_seq_set_data(
                                                ctx,
                                                it_partial->data.data(),
                                                checkpoint_size,
                                                slot.id,
                                                LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);

                                        if (n == checkpoint_size) {
                                            llama_pos restored_pos_next = std::max(it_partial->pos_min + 1, it_partial->pos_max);
                                            llama_pos restored_pos_next_prompt = std::max(it_partial->pos_min_prompt + 1, it_partial->pos_max_prompt);

                                            slot.n_past = slot.cache_tokens.size_up_to_pos(restored_pos_next);
                                            slot.n_past_prompt = slot.prompt_tokens.size_up_to_pos(restored_pos_next_prompt);
                                            slot.n_past_offset = slot.n_past_prompt - slot.n_past;
                                            slot.n_prompt_tokens_cache = slot.n_past_prompt;

                                            slot.cache_tokens.keep_first(slot.n_past);

                                            for (auto cit = slot.server_cached_prompt.checkpoints.begin();
                                                 cit != slot.server_cached_prompt.checkpoints.end();) {
                                                if (cit->pos_max > restored_pos_next) {
                                                    cit = slot.server_cached_prompt.checkpoints.erase(cit);
                                                } else {
                                                    ++cit;
                                                }
                                            }
                                            ...
                                            partial_reprefill_used = true;

                                            LLAMA_LOG_WARN("COMMON_PREFIX_MISMATCH_RECURRENT_PARTIAL_REPREFILL_MIN_V1: slot=%d task=%d mode=%s exact_prefix=%" PRId64 " selected_checkpoint=%" PRId64 " prompt_tokens=%d tokens_to_prefill=%d n_before_user=%d tool_edit_mismatch=%d action=allow_partial reason=safe_boundary_window_ok took=%.2fms\n",
                                                    slot.id,
                                                    slot.id_task,
                                                    params_base.ctx_checkpoints_safe_boundary_only ? "safe_boundary_window" : (single_slot_mode ? "nearest" : "n_before_user"),
                                                    exact_prefix,
                                                    ...
                                                    tokens_to_prefill,
                                                    slot.n_before_user,
                                                    tool_edit_mismatch ? 1 : 0,
                                                    (ggml_time_us() - t_start_partial) / 1000.0);
                                        } else {
                                            LLAMA_LOG_WARN("COMMON_PREFIX_MISMATCH_RECURRENT_PARTIAL_REPREFILL_MIN_V1_FAILED: slot=%d task=%d selected_checkpoint=%" PRId64 " checkpoint_size=%zu restored=%zu; falling back to clean prefill\n",
                                                    slot.id,
                                                    slot.id_task,
                                                    selected_checkpoint,
                                                    checkpoint_size,
                                    ...
                                    if (!partial_reprefill_used) {
                                        if (params_base.ctx_checkpoints_safe_boundary_only) {
                                            const bool cold_restart_mismatch =
                                                    exact_prefix >= 0 &&
                                                    exact_prefix < params_base.ctx_checkpoints_interval &&
                                                    slot.server_cached_prompt.checkpoints.size() <= 1;

                                            LLAMA_LOG_WARN("QWEN36_SAFE_BOUNDARY_ONLY_PARTIAL_FALLBACK_FULL_PREFILL_V1: slot=%d task=%d exact_prefix=%" PRId64 " min_window=%" PRId64 " max_checkpoint=%" PRId64 " checkpoint_count=%zu cold_restart=%d tool_edit_mismatch=%d action=full_clean_prefill reason=no_safe_boundary_window_candidate\n",
                                                    slot.id,
                                                    slot.id_task,
                                                    exact_prefix,
                                                    min_reprefill_window,
                                                    max_partial_checkpoint,
                                                    slot.server_cached_prompt.checkpoints.size(),
                                                    cold_restart_mismatch ? 1 : 0,
                                                    tool_edit_mismatch ? 1 : 0);

                                            if (cold_restart_mismatch) {
                                                LLAMA_LOG_WARN("QWEN36_SAFE_BOUNDARY_ONLY_COLD_RESTART_ANCHOR_REBUILD_V1: slot=%d task=%d exact_prefix=%" PRId64 " action=clean_prefill_then_rebuild_initial_anchor\n",
                                                        slot.id,
                                                        slot.id_task,
                                                        exact_prefix);
                                            }
                                        ...
                                        LLAMA_LOG_WARN("COMMON_PREFIX_MISMATCH_RECURRENT_REPREFILL: slot=%d task=%d; clearing recurrent checkpoints and forcing clean prefill\n",
                                                slot.id,
                                                slot.id_task);
                ...
                const bool trimmed = trim_slot_kv_or_clear_for_refill(slot, ctx, p0);
                if (!trimmed) {
                    // could not partially delete recurrent/hybrid state; slot was cleared locally
                ...
                while (slot.n_past_prompt < slot.n_prompt_tokens && batch.n_tokens < n_batch) {
                    ...
                    if (params_base.do_checkpoint &&
                            params_base.ctx_checkpoints_safe_boundary_only &&
                            slot.pending_periodic_safe_boundary_checkpoint &&
                            slot.pending_periodic_safe_boundary_min_pos >= 0) {
                        while (!slot.n_before_user_boundaries.empty() &&
                                slot.n_before_user_boundaries.front() < slot.pending_periodic_safe_boundary_min_pos) {
                            SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_SKIP_STALE_BOUNDARY_V1: slot=%d task=%d boundary=%d min_boundary=%d tag=%s\n",
                                    slot.id,
                                    slot.id_task,
                                    slot.n_before_user_boundaries.front(),
                                    slot.pending_periodic_safe_boundary_min_pos,
                                    !slot.n_before_user_boundary_tags.empty() ? slot.n_before_user_boundary_tags.front().c_str() : "unknown");

                            slot.n_before_user_boundaries.erase(slot.n_before_user_boundaries.begin());
                            if (!slot.n_before_user_boundary_tags.empty()) {
                                slot.n_before_user_boundary_tags.erase(slot.n_before_user_boundary_tags.begin());
                            }
                        }
                    ...
                    if (params_base.do_checkpoint &&
                            slot.n_before_user > 0 &&
                            slot.n_past_prompt >= slot.n_before_user &&
                            (!params_base.ctx_checkpoints_safe_boundary_only ||
                             slot.pending_periodic_safe_boundary_checkpoint ||
                             slot.server_cached_prompt.checkpoints.empty())) {
                        const bool safe_boundary_initial_anchor_checkpoint =
                                params_base.ctx_checkpoints_safe_boundary_only &&
                                slot.server_cached_prompt.checkpoints.empty() &&
                                !slot.pending_periodic_safe_boundary_checkpoint;
                        ...
                        slot.pending_checkpoint_metadata_valid = true;
                        slot.pending_checkpoint_safe_boundary_created = true;
                        slot.pending_checkpoint_type = slot.n_before_user_tag.empty()
                            ? "user_boundary"
                            : slot.n_before_user_tag;
                        slot.pending_checkpoint_source = params_base.ctx_checkpoints_safe_boundary_only
                            ? "message_spans_periodic"
                            : "message_spans";

                        const size_t checkpoint_count = slot.server_cached_prompt.checkpoints.size();
                        const size_t checkpoint_limit = std::max<size_t>(1, (size_t) params_base.ctx_checkpoints_n);

                        if (params_base.ctx_checkpoints_safe_boundary_only) {
                            slot.forced_checkpoint_min_step = 0;
                        } else if (checkpoint_count + 2 < checkpoint_limit) {
                            slot.forced_checkpoint_min_step = 128;
                        } else if (checkpoint_count + 1 < checkpoint_limit) {
                            slot.forced_checkpoint_min_step = 256;
                        } else {
                            slot.forced_checkpoint_min_step = 256;
                        }

                        while (!slot.n_before_user_boundaries.empty() &&
                                slot.n_before_user_boundaries.front() <= slot.n_past_prompt) {
                            slot.n_before_user_boundaries.erase(slot.n_before_user_boundaries.begin());
                            if (!slot.n_before_user_boundary_tags.empty()) {
                                slot.n_before_user_boundary_tags.erase(slot.n_before_user_boundary_tags.begin());
                            }
                        ...
                        if (!slot.n_before_user_boundaries.empty()) {
                            ...
                            slot.n_before_user_tag = !slot.n_before_user_boundary_tags.empty()
                                ...
                                : "user_boundary";
                            slot.n_before_user_known = true;
                            SRV_INF("message_spans: slot=%d next n_before_user=%d tag=%s remaining_boundaries=%zu\n",
                                    slot.id, slot.n_before_user, slot.n_before_user_tag.c_str(), slot.n_before_user_boundaries.size());
                        } else {
                            slot.n_before_user = -1;
                            slot.n_before_user_tag.clear();
                            slot.n_before_user_known = false;
                        ...
                        if (params_base.ctx_checkpoints_safe_boundary_only) {
                            SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_ANCHOR_ON_CROSSING_V1: slot=%d task=%d action=batch_cut checkpoint_type=%s source=%s n_past_prompt=%d n_past=%d initial_anchor=%d pending_periodic=%d checkpoint_count=%zu\n",
                                    slot.id,
                                    slot.id_task,
                                    slot.pending_checkpoint_type.c_str(),
                                    slot.pending_checkpoint_source.c_str(),
                                    slot.n_past_prompt,
                                    slot.n_past,
                                    safe_boundary_initial_anchor_checkpoint ? 1 : 0,
                                    slot.pending_periodic_safe_boundary_checkpoint ? 1 : 0,
                                    slot.server_cached_prompt.checkpoints.size());
                        }
                    ...
                    if (params_base.do_checkpoint && !params_base.ctx_checkpoints_safe_boundary_only) {
                        // If a chat/user boundary is known and still pending, skip ordinary
                        // prompt-tail/tolerance checkpoints until the boundary checkpoint is created.
                        // This keeps the first semantic checkpoint aligned with n_before_user.
                        const bool skip_ordinary_prompt_checkpoint =
                            slot.n_before_user_known && slot.n_before_user > 0;

                        if (!skip_ordinary_prompt_checkpoint) {
                            // LLAMA_UPSTREAM: create prompt-tail checkpoints close to the end of the prompt.
                            // llama.cpp modern checkpointing uses offsets {4 + n_ubatch, 4} so the server
                            // can restore near the prompt tail instead of relying on a single hardcoded point.
                            const int32_t n_remaining_prompt = slot.n_prompt_tokens - slot.n_past_prompt;
                            const int32_t checkpoint_offsets[] = {4 + n_ubatch, 4};

                            for (const int32_t offset : checkpoint_offsets) {
                                if (n_remaining_prompt == offset) {
                                    slot.do_checkpoint = true;
                                    slot.forced_checkpoint_min_step = 256;
                                    break;
                                }
                            ...
                            // Keep ik_llama.cpp legacy tolerance behavior as an additional trigger.
                            if (!slot.do_checkpoint && params_base.ctx_checkpoints_tolerance > 0 &&
                                    n_remaining_prompt == params_base.ctx_checkpoints_tolerance) {
                                slot.do_checkpoint = true;
                                slot.forced_checkpoint_min_step = 256;
                            }
                        }
                ...
                // Qwen3Next / hybrid recurrent models are very sensitive to mixed-sequence
                // prompt batches. Upstream llama.cpp handles this with recurrent/hybrid
                // split_seq ubatches. ik_llama.cpp does not have that allocator layer here,
                // so prevent the server from appending prompt tokens from another slot into
                // the same batch once this slot contributed prompt tokens.
                if (llama_model_has_recurrent(model) && slot.n_prompt_tokens_processed > 0) {
                    break;
                }
...
void server_context::release_slot_after_final_response(server_slot & slot) {
    ...
    } else if (params_base.do_checkpoint && params_base.ctx_checkpoints_safe_boundary_only) {
        SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_SKIP_BASE_CHECKPOINT_V1: slot=%d task=%d reason=final_response\n",
                slot.id,
                slot.id_task);
    }
    slot.release();
        ...
        for (auto& slot : slots) {
            ...
            if (!is_active_slot || slot.i_batch < (int)i || slot.i_batch >= (int)(i + n_tokens)) {
                ...
                if (slot.command == SLOT_COMMAND_LOAD_PROMPT) {
                    if (slot.do_checkpoint) {
                        create_checkpoint(slot);
                    } else if (!(slot.n_before_user_known && slot.n_before_user > 0)) {
            ...
            if (slot.n_decoded == 1) {
                ...
                if (params_base.ctx_checkpoints_tolerance<=0 && params_base.do_checkpoint && !params_base.ctx_checkpoints_safe_boundary_only) {
                    create_checkpoint(slot);
                } else if (params_base.ctx_checkpoints_tolerance<=0 && params_base.do_checkpoint && params_base.ctx_checkpoints_safe_boundary_only) {
                    SRV_INF("QWEN36_SAFE_BOUNDARY_ONLY_SKIP_BASE_CHECKPOINT_V1: slot=%d task=%d reason=prompt_end\n",
                            slot.id,
                            slot.id_task);
                }
            }
