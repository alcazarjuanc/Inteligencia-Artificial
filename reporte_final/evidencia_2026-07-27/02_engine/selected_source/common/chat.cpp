...
static std::string qwen36_canonicalize_edit_arguments_v1(const std::string & name, const std::string & arguments) {
    if (name != "Edit") {
        return arguments;
    }

    try {
        json in = json::parse(arguments);
        ...
        if (in.contains("file_path")) {
            out["file_path"] = in["file_path"];
        }
        if (in.contains("old_string")) {
            out["old_string"] = in["old_string"];
        }
        if (in.contains("new_string")) {
            out["new_string"] = in["new_string"];
        }
        if (in.contains("replace_all") && !(in["replace_all"].is_boolean() && in["replace_all"] == false)) {
            out["replace_all"] = in["replace_all"];
        }

        for (const auto & item : in.items()) {
            const std::string & key = item.key();
            if (key != "file_path" && key != "old_string" && key != "new_string" && key != "replace_all") {
                out[key] = item.value();
            }
        }
    ...
    if (!tool_calls.empty()) {
        ...
        for (const auto & tool_call : tool_calls) {
            const std::string canonical_arguments =
                    qwen36_canonicalize_edit_arguments_v1(tool_call.name, tool_call.arguments);
            LOG_WRN("QWEN36_TOOL_ARG_ROUNDTRIP_TO_JSON_V1: name=%s raw_arguments=[%s] canonical_arguments=[%s]\n",
                    tool_call.name.c_str(), tool_call.arguments.c_str(), canonical_arguments.c_str());
            json tc {
                {"type", "function"},
                {"function", {
                    {"name", tool_call.name},
                    {"arguments", json(canonical_arguments)},
                }},
            };
        ...
        for (const auto & message : messages) {
            ...
            if (has_tool_calls) {
                for (const auto & tool_call : message.at("tool_calls")) {
                    ...
                    const std::string raw_arguments_before_canonical = tc.arguments;
                    tc.arguments = qwen36_canonicalize_edit_arguments_v1(tc.name, tc.arguments);
                    LOG_WRN("QWEN36_TOOL_ARG_ROUNDTRIP_FROM_OAI_V1: name=%s args_is_string=%d raw_arguments=[%s] canonical_arguments=[%s] source_args_dump=[%s]\n",
                            tc.name.c_str(),
                            args.is_string() ? 1 : 0,
                            raw_arguments_before_canonical.c_str(),
                            tc.arguments.c_str(),
                            args.dump().c_str());
